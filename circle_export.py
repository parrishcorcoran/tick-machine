...
u_res = np.maximum(np.max(res, axis=0), 1e-6) / HALF                        # the residual stream's dial, per dimension
out = {"u_res": u_res}
...
for k in range(L):
    p = f"transformer.h.{k}."
    to_circle(f"qkv.{k}", sd[p+"attn.c_attn.weight"], sd[p+"attn.c_attn.bias"], HALF, HALF)
    to_circle(f"proj.{k}", sd[p+"attn.c_proj.weight"], sd[p+"attn.c_proj.bias"], HALF, HALF)
    to_circle(f"fc.{k}", sd[p+"mlp.c_fc.weight"], sd[p+"mlp.c_fc.bias"], HALF, HALF)
