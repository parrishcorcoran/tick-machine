...
int main(int argc,char**argv){
  ...
  if(pid==0){ alarm(5); long seen=0; int k=0;
    double tk=tick; while(k<N+10 && seen<N+10){ long f; double te=ts[k>0?k-1:0]; int m=0; while((f=*flag)==seen){ double now=ns(); int slot=(int)((now-te)/tk); if(k>0 && slot>=1 && slot<=4 && slot>m){ lv_seen[(k-1)*8+slot]=*level; m=slot; } } dir[k]=f-seen; seen=f; ts[k++]=ns(); } _exit(0); }
  ...
  for(int i=0;i<N;i++){
    if(mode){ double t_edge=at; at += (step[i]+1+GUARD_T)*tick;
      for(int b=1;b<=4;b++){ while(ns()<t_edge+(b-0.5)*tick); *level = (lapbits[i]>>(b-1))&1 ? +1 : -1; }   // rest at +0/-0 for 4 idle ticks after the last edge
      while(ns()<at); *flag = ++e; }
  ...
  static double tsa[N+12]; static int have[N+12]; { long c=0; for(int k=0;k<N+10;k++){ c+=dir[k]; if(c>=1&&c<=N+10){ tsa[c-1]=ts[k]; have[c-1]=1; } } }
  int missed=0; for(int k=0;k<N+10;k++) if(!have[k]) missed++;
  double acc=0, ref=0; int bad=0, maxoff=0, lapbad=0;
  for(int i=0;i<N;i++){ int got=0; for(int b=1;b<=4;b++) got |= (lv_seen[(i+9)*8+b]>0)<<(b-1); if(got!=lapbits[i]) lapbad++; }
  for(int i=0;i<N;i++){ int got;
    if(mode){ if(!have[i+10]){ got=q[order[i]]; bad++; continue; } double tt=(tsa[i+10]-tsa[9])/tick; got=(int)floor(tt+0.5)-(i+1)*(1+GUARD_T)-HALF; }   // nominal tick (same clock both ends), from the last warm pilot
  ...
  printf("lap counters wrong %d | missed edges %d | tick %4.0f ns  %-10s: %d wrong of %d (worst off by %d ticks)   dot %.5f vs %.5f (rel err %.1e)   %.0f weights/s on one line\n",
    lapbad, missed, tick, mode?"sorted steps":"flat 4096", bad, N, maxoff, acc, ref, fabs(acc-ref)/fabs(ref), N/secs);
  return 0; }
